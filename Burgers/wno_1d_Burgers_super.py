...
class WaveConv1d(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[-1] > self.size:
            ...
            # Compute single tree Discrete Wavelet coefficients using some wavelet  
            dwt = DWT1D(wave=self.mother_wavelet, J=self.level+factor, mode=self.mode).to(x.device)
            x_ft, x_coeff = dwt(x)
            ...
            # Compute single tree Discrete Wavelet coefficients using some wavelet  
            dwt = DWT1D(wave=self.mother_wavelet, J=self.level, mode=self.mode).to(x.device)
            x_ft, x_coeff = dwt(x)
...
def train_loop(x_train, train_loader,test_loader,li):
    model = WNO1d(width, level, x_train.permute(0,2,1),li).to(device)
