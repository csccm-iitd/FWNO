...
dataloader = MatReader('data/burgers_data_N5000.mat')
...
def reward_function(architecture, epochs = 100):
    model=train_loop(x_train, train_loader,test_loader, architecture, epochs, ntrain = 5000)
...
def main():
    ...
    def training():
        ...
        for state in topk_states:
            model=train_loop(x_train, train_loader,test_loader, state, epochs = 500, ntrain = 5000)
            loss=test_loop(x_test, y_test, model)
            if loss<min_loss:
                min_loss = loss
                best_architecture = state
                best_model = model
        torch.save(best_model.state_dict(),'FWNO_Burgers_LargeDataset.pth')
        print(f'Best architecture searched for Burgers Equation with Large Dataset is {best_architecture}')
