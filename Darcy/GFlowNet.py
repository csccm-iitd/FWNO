...
class GFlowNet_wavelets(nn.Module):
    def __init__(self):
        super().__init__()
        self.layers=nn.Sequential(nn.Linear(8,128),nn.LeakyReLU(),nn.Linear(128,len(wavelets)))
...
class GFlowNet_activations(nn.Module):
    def __init__(self):
        super().__init__()
        self.layers=nn.Sequential(nn.Linear(8,128),nn.LeakyReLU(),nn.Linear(128,len(activations)))
        self.initialize_weights(4, -40)
    def initialize_weights(self,a,b):
            for m in self.layers:
                if isinstance(m, nn.Linear):
    ...
    def training():
        ...
        for state in topk_states:
            ...
            if loss<min_loss:
                ...
                best_model = model
        torch.save(best_model.state_dict(),'FWNO_Darcy.pth')
        print(f'Best architecture searched for Darcy Equation is {best_architecture}')
        ...
        # torch.save(model_wno.state_dict(), 'WNO_Darcy.pth')
        ...
        #                 extent=[0,1,0,1], interpolation='Gaussian', cmap='inferno', vmin=0, vmax=4e-4)
        # plt.colorbar(im, ax=ax[1,2], fraction=0.2, format='%.0e')
        ...
        #                 extent=[0,1,0,1], interpolation='Gaussian', cmap='inferno', vmin=0, vmax=4e-4)
        # plt.colorbar(im, ax=ax[1,3], fraction=0.2, format='%.0e')
        ...
        #                 extent=[0,1,0,1], interpolation='Gaussian', cmap='inferno', vmin=0, vmax=3e-4)
        # plt.colorbar(im, ax=ax[3,2], fraction=0.2, format='%.0e')
        ...
        #                 extent=[0,1,0,1], interpolation='Gaussian', cmap='inferno', vmin=0, vmax=3e-4)
        # plt.colorbar(im, ax=ax[3,3], fraction=0.2, format='%.0e')
